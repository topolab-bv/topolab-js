...
describe("dataset", () => {
  ...
  it("items() is slug-direct (no metadata round-trip) and serializes bbox", async () => {
    let mdCalls = 0;
    const bboxSeen: (string | null)[] = [];
    server.use(
      // The collectionId IS the slug, so items() must NOT hit the metadata
      // endpoint. If it ever does, this handler fires and the assertion fails.
    ...
    expect(mdCalls).toBe(0); // collectionId IS the slug — no metadata round-trip
